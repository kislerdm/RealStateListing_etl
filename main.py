...
global file_config, db_connection
verbose = True
...
def main():
    ...
    while True:
        # if stop.dat file is present process needs to shut down, we will be checking this only at the start of loop.
        for file in os.listdir(file_config['filepath']):
            if 'stop.dat' in file:
                logger.info("Exiting as shutdown command is called.")
                os.remove(os.path.join(file_config['filepath'], 'stop.dat'))
                return

            if file_config['fileprefix'] in file:
                print(file)
                source_file = os.path.join(file_config['filepath'], file)
                target_file = os.path.join(file_config['processing_directory'], file)
...
def init():
    """
    function to get database and input file parameters from config
    ...
    file_config = conf['Input-file']
    db_connection = datalayer.databaseConnection(conf['database-connection'])
...
def main_parser(file_name):
    ...
    with open(file_name,'r') as input_file:
        for count,line in enumerate(input_file):
            ...
            input_data = parser.json_extract(line)
            logger.info("Input data : {}".format(input_data)) # writing it to logger for future use

            """
            Below line will extract only required fields from the input based on mapping.txt.
            if required field is not present will assign default value to the attribute.
            """
            column_data = parser.get_formatted_dict(input_data)
            column_data = parser.business_logic(column_data) #applying transformation on input data as mentioned in parser.business_logic

            """
            below function extract the query from parser module, this query already have all the bind attributes 
            mentioned and database will get those attributes from column_data dictionary.
            """
            query = parser.get_insert_query()
            db_connection.query(query, column_data)
            if count%100 == 0 :
                print("processed  {} records ".format(count))


    db_connection.connection.commit() #TODO have commit based on batch size and have recovery logic.
